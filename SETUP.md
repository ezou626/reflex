...
Training can be launched with the `train.sh` script, the repo contains our base model in model/library.json (which can be used directly without training). Running the training script
provides an '-r' flag to reset the model and use entirely new training workloads.

Workloads can individually be trained by running tune_experiment.py on a given workload, or by running the train script which can do several predefined workloads in succession.
