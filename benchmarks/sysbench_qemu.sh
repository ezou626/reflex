#!/usr/bin/env bash
# Boot an Ubuntu cloud image under QEMU, build the local eBPF loader, install
# sysbench, and run benchmarks/sysbench_compare.sh inside the guest.
#
# Usage:
#   MODES=workload_only,heuristic benchmarks/sysbench_qemu.sh --test cpu
#
# Useful env:
...
#   MODES                 comma modes for sysbench_compare.sh
set -euo pipefail
...
OVERLAY="${CACHE_DIR}/sysbench-overlay-$$.qcow2"
SEED_ISO="${CACHE_DIR}/sysbench-seed-$$.img"
KEY="${CACHE_DIR}/id_ed25519"
PUB="${KEY}.pub"
CONSOLE_LOG="${CACHE_DIR}/sysbench-console-$$.log"
QEMU_PID_FILE="${CACHE_DIR}/sysbench-qemu-$$.pid"
KNOWN_HOSTS="${CACHE_DIR}/known_hosts.sysbench.$$"
...
if [[ ! -e /dev/kvm ]]; then
  echo "error: /dev/kvm does not exist; enable KVM or nested virtualization." >&2
  exit 1
fi
if [[ ! -r /dev/kvm ]]; then
...
if [[ ! -f "${KEY}" ]]; then
  echo "[sysbench-qemu] generating SSH key: ${KEY}"
  ssh-keygen -t ed25519 -f "${KEY}" -N "" -q
fi

if [[ ! -f "${BASE_IMG}" ]]; then
  echo "[sysbench-qemu] downloading ${BASE_URL}"
...
META_DATA="$(mktemp)"
USER_DATA="$(mktemp)"
INSTANCE_ID="reflex-sysbench-$(date +%s)-$$"
cat >"${META_DATA}" <<EOF
instance-id: ${INSTANCE_ID}
local-hostname: reflex-sysbench
EOF
...
echo "[sysbench-qemu] starting QEMU on 127.0.0.1:${SSH_PORT}"
qemu-system-x86_64 \
...
echo "[sysbench-qemu] waiting for SSH"
READY=0
...
THREADS_GUEST_ARG=""
if [[ -n "${THREADS}" ]]; then
  printf -v THREADS_GUEST_ARG ' --threads %q' "${THREADS}"
fi
DRY_RUN_GUEST_ARG=""
if [[ "${#DRY_RUN_ARG[@]}" -gt 0 ]]; then
  DRY_RUN_GUEST_ARG=" --dry-run"
fi
printf -v GUEST_EXTRA '%q ' "${EXTRA_ARGS[@]}"
REMOTE_ENV="MODES='${MODES:-workload_only,heuristic,classifier}'"
REMOTE_ENV+=" SYSBENCH_TEST='${TEST}' SYSBENCH_TIME='${TIME_SEC}'"

echo "[sysbench-qemu] running guest benchmark"
ssh \
  -i "${KEY}" \
  -p "${SSH_PORT}" \
...
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
  ...
  git make curl ca-certificates linux-tools-common sysbench >/dev/null
...
RUN_ROOT="/mnt/reflex/data/runs/sysbench-\${SYSBENCH_TEST}-\$(date +%Y%m%d-%H%M%S)"
RUN_ROOT="\${RUN_ROOT}" MODES="\${MODES}" bash benchmarks/sysbench_compare.sh \
  --test "\${SYSBENCH_TEST}" --time "\${SYSBENCH_TIME}"${THREADS_GUEST_ARG}${DRY_RUN_GUEST_ARG} -- ${GUEST_EXTRA}
GUEST

echo "[sysbench-qemu] complete"
